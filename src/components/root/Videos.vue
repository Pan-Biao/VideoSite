...
const userSuspend = async (row) => {
  const type = await VXETable.modal.confirm("您确定要禁用此视频吗?");
  if (type === "confirm") {
    useVideoSuspend(row.id).then((res) => {
      getList();
    });
  }
};
...
const userUnseal = async (row) => {
  const type = await VXETable.modal.confirm("您确定要解禁此视频吗?");
  if (type === "confirm") {
    useVideoUnseal(row.id).then((res) => {
      getList();
    });
  }
};
...
<template>
  ...
  <div class="flex">
    ...
    >
      ...
      <vxe-column field="like_number" title="点赞数"></vxe-column>
      <vxe-column field="play_number" title="播放量"></vxe-column>
      <vxe-column field="said" title="分区id"></vxe-column>
      ...
      <vxe-column
        ...
        :formatter="formatTime"
      ></vxe-column>
      <vxe-column field="cover" title="封面" show-overflow></vxe-column>
      <vxe-column field="path" title="视频地址" show-overflow></vxe-column>
      <vxe-column title="操作" width="80" fixed="right" show-overflow>
        <template #default="{ row }">
          <vxe-button v-if="row.state" @click="userSuspend(row)"
            >禁用</vxe-button
          >
